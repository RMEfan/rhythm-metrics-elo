...
phase 2 - sort score vectors, calculate elo per month
  sort score history by datetime
  iteratively send map shards through elom in temporal order
    save ratings after each month to keep history
```{r}
...
#calc osu ratings for each month
osu_ratings_m3 <- NULL
start_year <- 2024
start_month <- 4
prev_time <- as.numeric(strptime(paste(
      start_year, "-", start_month, "-01 00:00:00", sep=""),
      format="%Y-%m-%d %H:%M:%S", tz="UTC"))
for (i in 2024:2024) {
  for (j in 5:5) {
    # find this month's map shard indices
    cur_time <- as.numeric(strptime(paste(
      i, "-", j, "-01 00:00:00", sep=""), 
      format="%Y-%m-%d %H:%M:%S", tz="UTC"))
    
    month_indices <- time_indices %>%
      filter(Datetime >= prev_time,
             Datetime < cur_time) %>%
      pull(Index)
    
    if (length(month_indices) == 0) next
    
    # send each index through elom
    for (map_index in month_indices) {
      scorevec <- score_history[[map_index]]
    ...
    # print to file
    write.csv(osu_ratings_m3$ratings, 
            paste("csvfiles/ratings_history/ratings_", i, "-", j, ".csv", sep=""), 
            row.names = FALSE)
    prev_time <- cur_time
  }
}
...
# load RME ratings for each year/month
for (year in years) {
  for (month in months) {
    rme_ratings <- read.csv(paste("csvfiles/ratings_history/ratings_", year, "-", month, ".csv", sep=""))
    
    #print(paste("hey", year, month, sep=" "))

    # fill in RME ratings
    month_data <- player_data %>%
      left_join(rme_ratings, by = join_by(UserID == Player)) %>%
      mutate(RME = Rating, RatingPeriod = as.Date(paste(year, month, "01", sep="-"))) %>%
      select(RatingPeriod, UserID, Username, GlobalRank, PP, RME, Location, RegistrationDate, Games)
    
    #print(month_data %>% slice_head(n=1))
    #next
    # delete NA values
    ...
    month_data <- month_data %>%
      arrange(desc(RME)) %>%
      mutate(RMERanking = 1:length(month_data$UserID), .after=RME)
    
    write.csv(month_data, paste("csvfiles/user_rating_history/users_with_RME_", 
                                 year, "-", month, 
...
combine rme history into one file
```{r}
files <- list.files(path = "csvfiles/user_rating_history/", pattern = "*.csv")

#print(files)

combined <- do.call(rbind, lapply(paste("csvfiles/user_rating_history/", files, sep=""), read.csv))

write.csv(combined, "Rating_history.csv", row.names = FALSE)
